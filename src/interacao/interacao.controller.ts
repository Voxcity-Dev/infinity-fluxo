...
import { ListInteracoesInput, ListInteracoesResponseDto, ListInteracoesSchema } from './dto/list-interacao.dto';
...
export class InteracaoController {
	...
	*/
	...
	@ApiResponse({ status: 401, description: 'Não autorizado' })
	@ApiResponse({ status: 422, description: 'Dados de validação inválidos' })
	async listar(@Body(new ZodPipe(ListInteracoesSchema)) params: ListInteracoesInput) {
	...
	@ApiResponse({ status: 401, description: 'Não autorizado' })
	@ApiResponse({ status: 422, description: 'Dados de validação inválidos' })
	async criar(@Body(new ZodPipe(CreateInteracaoSchema)) data: CreateInteracaoDto) {
		const interacao = await this.interacaoService.create(data);
